...
const ART_TIPS = ["Use specific adjectives to describe textures and materials", "Include lighting details like 'golden hour' or 'dramatic shadows'", "Specify art styles like 'oil painting' or 'digital art'", "Add atmospheric elements like 'misty', 'ethereal', or 'dramatic'"];
      ...
      <SheetContent className="w-[90vw] sm:w-[440px] h-[90vh] sm:h-[600px] flex flex-col p-0">
        ...
        <ScrollArea className="flex-1 p-3 sm:p-4">
          <div className="space-y-4">
            {messages.map((message, index) => <ChatMessage key={index} message={message.text} isBot={message.isBot} />)}
          </div>
        </ScrollArea>
        <div className="p-3 sm:p-4 border-t">
          <div className="flex gap-2">
            <Input placeholder="Ask about prompts, tips, or troubleshooting..." value={input} onChange={e => setInput(e.target.value)} onKeyPress={handleKeyPress} className="flex-1 text-sm sm:text-base" disabled={isLoading} />
            <Button size="icon" onClick={handleSend} disabled={!input.trim() || isLoading} className="bg-amber-500 hover:bg-amber-600 min-w-[40px]" aria-label="Send message">
